...
<body>
	...
	<c:choose>
		<%-- 
			...
			2) 연산 대상이 배열/컬렉션인데
			  ...
			  <c:whon test="${param.age}}">
		 --%>
		<c:when test="${empty param.age}">
			<h4 stlye="color:red">age 값이 없습니다</h4>
		</c:when>
	...
	<ul>
		<c:forEach var="name" items="${nameList}" varStatus="vs">
			...
			<c:if test="${vs.first}"> <%-- 반복문의 첫번쨰 순회일 때 true --%>
				<li>-------------------시작-------------------</li>
			</c:if>
			<li style="margin-bottom: 10px;">
				<ul>
					...
					<%-- 
						...
						1) ${컬렉션|배열[index]}
						2) varStatus.current
						3) forEach에 var 속성에 작성한 변수명 세팅 후
						   ${변수명}
					 --%>
			...
			<c:if test="${vs.last}"> <%-- 반복문의 첫번쨰 순회일 때 true --%>
				<li>-------------------종료-------------------</li>
			</c:if>
		</c:forEach>
	</ul>
